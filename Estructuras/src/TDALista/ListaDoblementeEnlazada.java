...
public class ListaDoblementeEnlazada<E> implements PositionList<E> {
	...
	/**
	 * Inicializa una lista doblemente enlazada con tamaño 0, cabeza nula y rabo nulo.
	 */
	...
	public Iterable<Position<E>> positions() {
		...
		try{
			...
			while(p!=rabo.getAnterior()) {//mientras p sea diferente al çultimo elemento
				l.addLast(p);
				p=next(p);
			}
			l.addLast(p);
	...
	public void invertir() {
		try {
			if (tamaño>1) {
				...
				for (int i=0;i<tamaño-1;i++) {//agrego el anteúltimo despues del último, menos el ultimo original de la lista
